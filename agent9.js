...
CasperRenderer.prototype.writeHeader = function() {
  ...
  //this.text("/*==============================================================================*/", 0);
  //this.space();
  //this.stmt("var xPathLocator = require('casper').selectXPath;", 0);
  this.document.write("Agent 9 Script:<br><textarea rows='50' cols='250' id='textcode'></textarea>");
}
...
CasperRenderer.prototype.startUrl = function(item) {
  ...
  this.stmt("<STEP>",0);
  this.space();
  this.stmt("<ACTION><![CDATA[",1);
  this.space();
  this.stmt("//this.open(" + url + ");",2);
  this.space();
...
CasperRenderer.prototype.click = function(item) {
  ...
  if(this.with_xy && !(tag == 'a' || tag == 'input' || tag == 'button')) {
    ...
    if (tag == 'a') {
      ...
      if(xpath_selector) {
        selector = 'xPathLocator("//a['+xpath_selector+']")';
      } else {
        selector = item.info.selector;
      }
	...
	this.stmt('this.then(function() {',2);
	this.space();
    this.stmt('this.click('+ selector + ');',3);
	this.space();
    this.stmt('});',2);
	this.space();
  }
...
CasperRenderer.prototype.checkTextPresent = function(item) {
    var selector = 'xPathLocator("//*[contains(text(), '+this.pyrepr(item.text, true)+')]")';
...
CasperRenderer.prototype.checkValue = function(item) {
  ...
  if (type == 'checkbox' || type == 'radio') {
    ...
    selector = 'xPathLocator("//input[' + way + ' and ' +selected+ ']")';
  }
  else {
    var value = this.pyrepr(item.info.value)
    var tag = item.info.tagName.toLowerCase();
    selector = 'xPathLocator("//'+tag+'[' + way + ' and @value='+value+']")';
  }
  this.waitAndTestSelector(selector);
}
...
CasperRenderer.prototype.checkText = function(item) {
  ...
  if ((item.info.type == "submit") || (item.info.type == "button")) {
      selector = 'xPathLocator("//input[@value='+this.pyrepr(item.text, true)+']")';
  } else {
      selector = 'xPathLocator("//*[normalize-space(text())='+this.cleanStringForXpath(item.text, true)+']")';
  }
  this.waitAndTestSelector(selector);
}
...
CasperRenderer.prototype.checkHref = function(item) {
  ...
  var xpath_selector = this.getLinkXPath(item);
  if(xpath_selector) {
    selector = 'xPathLocator("//a['+xpath_selector+' and @href='+ href +']")';
  } else {
...
CasperRenderer.prototype.checkEnabled = function(item) {
    var way = this.getControlXPath(item);
    var tag = item.info.tagName.toLowerCase();
    this.waitAndTestSelector('xPathLocator("//'+tag+'[' + way + ' and not(@disabled)]")');
}

CasperRenderer.prototype.checkDisabled = function(item) {
  var way = this.getControlXPath(item);
  var tag = item.info.tagName.toLowerCase();
  this.waitAndTestSelector('xPathLocator("//'+tag+'[' + way + ' and @disabled]")');
}

CasperRenderer.prototype.checkSelectValue = function(item) {
  var value = this.pyrepr(item.info.value);
  var way = this.getControlXPath(item);
  this.waitAndTestSelector('xPathLocator("//select[' + way + ']/options[@selected and @value='+value+']")');
}
...
CasperRenderer.prototype.checkImageSrc = function(item) {
  ...
  this.waitAndTestSelector('xPathLocator("//img[@src=' + src + ']")');
}
